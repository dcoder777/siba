<?php
declare(strict_types=1);
?>
<!doctype html>
<html lang="en">
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width,initial-scale=1">
    <title>SIBA Public School ERP</title>
    <link rel="stylesheet" href="./assets/vendor/bootstrap/bootstrap.min.css">
    <link rel="stylesheet" href="./assets/erp-ui.css">
</head>
<body class="page-shell">
<div class="content-wrap welcome-shell">
    <section class="surface-card hero-card">
        <div class="hero-grid">
            <div class="hero-panel stack" style="justify-content:center;align-items:center;text-align:center;">
                <div class="stack" style="gap:.8rem">
                    <h1 style="font-size:2.5rem;margin:0;">SIBA PUBLIC SCHOOL<br>MANAGEMENT SYSTEM</h1>
                </div>

                <div class="hero-actions" style="margin-top:1.5rem;">
                    <a class="btn" href="./admin/login.php">Login to Dashboard</a>
                </div>
            </div>

            <aside class="hero-side stack" style="justify-content:space-between">
                <div class="stack" style="gap:.8rem">
                    <span class="eyebrow" style="background:rgba(255,255,255,.14);color:#f1fff6">Platform Highlights</span>
                    <h2 style="font-size:2rem;color:#fff">Built for daily school operations, not just data entry.</h2>
                    <p style="color:#d7ece0">
                        A cleaner ERP experience for staff, management, and future mobile users.
                    </p>
                </div>

                <div class="metric-list">
                    <div class="metric">
                        Attendance
                        <strong>Live Ready</strong>
                    </div>
                    <div class="metric">
                        Finance
                        <strong>Tracked</strong>
                    </div>
                    <div class="metric">
                        Operations
                        <strong>Connected</strong>
                    </div>
                    <div class="metric">
                        HR & Payroll
                        <strong>Managed</strong>
                    </div>
                </div>
            </aside>
        </div>
    </section>
</div>
</body>
</html>
